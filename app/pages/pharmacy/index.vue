  ...
  <div class="pb-[72px] min-h-screen bg-[#fafafa]">
    <div class="bg-white px-4 pt-12 pb-4">
      <div class="flex items-center justify-between mb-4">
        <div>
          <p class="text-sm text-[#969696]">Pharmacy</p>
          <p class="text-base font-semibold text-[#191919]">Order medicines & health products</p>
        </div>
      ...
      <button class="flex items-center gap-3 bg-[#f5f5f5] rounded-full px-4 py-3 w-full text-left">
        ...
        <span class="text-sm text-[#969696]">Search medicines, health products</span>
      </button>
    </div>
    ...
    <div class="flex gap-2 overflow-x-auto scrollbar-none px-4 pt-4 pb-1">
      <button
        v-for="cat in pharmacyCategories"
        :key="cat"
    ...
    <div class="px-4 pt-4 space-y-3">
      ...
      <div v-else-if="!stores.length" class="flex flex-col items-center justify-center py-24 text-center">
        <div class="w-24 h-24 rounded-full bg-brand-50 flex items-center justify-center mb-5">
          <UIcon name="i-lucide-pill" class="w-10 h-10 text-brand-500" />
        </div>
        <h2 class="text-xl font-bold text-[#191919] mb-2">No pharmacies found</h2>
        <p class="text-sm text-[#969696] max-w-[260px]">Check back soon — more pharmacies are being onboarded.</p>
      </div>
      ...
      >
        <div class="w-full h-[168px] bg-green-50 relative flex items-center justify-center">
          <img v-if="store.image_url" :src="store.image_url" class="w-full h-full object-cover" />
          <UIcon v-else name="i-lucide-pill" class="w-12 h-12 text-green-400" />
        ...
        <div class="px-3 py-3">
          ...
          <div class="flex items-center gap-3 mt-1.5">
            <div class="flex items-center gap-1 text-[10px] text-[#969696]">
              ...
              {{ store.average_prep_time_minutes ?? 20 }} Mins
            </div>
            <div class="flex items-center gap-1 text-[10px] text-[#969696]">
...
const pharmacyCategories = ['All', 'Prescription Drugs', 'OTC Medicines', 'Vitamins & Supplements', 'Personal Care', 'Baby & Mother']
const selectedCat = ref('All')

const { data, pending } = await useAsyncData('pharmacies', () => api.getPharmacies() as any, { server: false })
const stores = computed<any[]>(() => (data.value as any)?.data ?? [])

useSeoMeta({ title: 'Pharmacy — NearbyMe' })
</script>
