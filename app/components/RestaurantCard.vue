<template>
  <!-- Wrapper div holds both the link and the heart; heart sits on top via absolute -->
  <div class="relative bg-white rounded-2xl overflow-hidden shadow-sm">
    <NuxtLink :to="`/restaurant/${restaurant.id}`" class="block">
      ...
      <div class="w-full h-[168px] bg-[#f5e9e7] relative flex items-center justify-center">
        ...
        <!-- Closed badge -->
        <div v-if="restaurant.is_open === false" class="absolute inset-0 bg-black/40 flex items-center justify-center">
      ...
      <div class="px-3 py-3">
        <div class="flex items-start justify-between gap-2">
          ...
          <div class="flex items-center gap-0.5 shrink-0">
            <UIcon name="i-lucide-star" class="w-3.5 h-3.5 text-[#f8cc6b]" style="fill: #f8cc6b" />
    ...
    <!-- Heart button — outside the NuxtLink so no navigation triggers -->
    <button
      class="absolute top-3 right-3 z-10 w-8 h-8 rounded-full flex items-center justify-center shadow-sm transition-colors"
    ...
    if (wasLiked) {
      if (!currentFavId) {
        ...
        const match = (list.data ?? []).find(
          (f: any) => f.type === 'vendor' && f.restaurants?.id === props.restaurant.id
        )
...
<style scoped>
/* Force the SVG path fill when heart is active (Lucide uses stroke-only paths) */
